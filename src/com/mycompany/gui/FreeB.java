 ...
 * @author Zahra
 */
public class FreeB extends BaseForm{
    
       public final TextField searchField = new TextField("", "Search");
  public FreeB(Resources res) {
        super("Offers Back office", BoxLayout.y());
        ...
        searchField.addDataChangedListener( (i1, i2) -> {
            ...
            ArrayList<Freelance> filteredCourses = new ArrayList<>();
           for (Freelance course : Service_Freelance.getInstance().findAll()) {
                if (course.getCategoryF().toLowerCase().contains(searchQuery) ||
                        course.getEmailbo().toLowerCase().contains(searchQuery)) {
        ...
        showCourses( Service_Freelance.getInstance().findAll());
    ...
    private void showCourses(ArrayList<Freelance> courses) {
        removeAll();
        createLineSeparator();
        Label label = new Label("\n ");
        Label label1 = new Label("\n ");
        addAll(label, label1,searchField);
        for (Freelance course : courses) {
            ...
            add(rowContainer);
            Label courseName = new Label(course.getCategoryF());
            Label courseDescription = new Label(course.getEmailbo());
            ...
            delete.addActionListener(evt -> {
                ...
                if (userResponse) {
                    Service_Freelance.getInstance().deleteFreelance(course.getIdbo());
